...
def format_math_expressions(text):
    # # Обрабатываем выражения в квадратных скобках [], убираем скобки и обрамляем знаком $
    # text = re.sub(r'\[(.*?)\]', r'$\1$', text)
    # text = re.sub(r'\[\s*(.*?)\s*\]', r'$\1$', text, flags=re.DOTALL)
    # # Убираем пробелы перед знаком $ в начале выражения
    # text = re.sub(r'\s*\$(.*?)$', r'$\1$', text)
    # # Убираем пробелы после знака $ в конце выражения
    # text = re.sub(r'^\$(.*?)\s*$', r'$\1$', text)
    # # Убираем пробелы до и после $ в уже заключенных выражениях, если они находятся внутри текста
    # text = re.sub(r'\s*\$\s*', '$', text)
    #
    # # Обрабатываем выражения в круглых скобках (), убираем скобки и обрамляем знаком $
    # text = re.sub(r'\\\((.*?)\\\)', r'$\1$', text)
